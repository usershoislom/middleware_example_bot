...
class FirstInnerMiddleware(BaseMiddleware):
    ...
    async def __call__(self,
        ...
        logger.debug("Выходим из миддлаварь %s", __class__.__name__)
...
class SecondInnerMiddleware(BaseMiddleware):
    ...
    async def __call__(self,
        ...
        logger.debug("Вошли в миддлаварь %s, тип события %s",
                     __class__.__name__,
                     event.__class__.__name__
        )
        result = handler(event, data)

        logger.debug("Выходим из миддлварь %s", __class__.__name__)
